...
if (isset($_POST['cancel']) ){
    ...
    $query =  mysqli_query($link,("DELETE FROM `tblleave` WHERE (LEAVESTATUS='INPROCESS' OR LEAVESTATUS='EDITED_REQUEST') AND (EMPID = '".$_SESSION['EMPID']."' AND LEAVEID = '".$_POST['LEAVEID']."')"));
...
<html>
    ...
    <body>
      ...
      <div class="topbar">
        ...
        <div class="content">
            ...
            <ul class="nav nav-tabs">
              <li class="nav-item">
                <a class="nav-link active" href="leavestatus.php">In-process</a>
              </li>
              <li class="nav-item">
     ...
     $query =  mysqli_query($link,("SELECT * FROM `tblleave` WHERE  EMPID = '".$_SESSION['EMPID']."' AND (LEAVESTATUS='INPROCESS' OR LEAVESTATUS='EDITED_REQUEST') ORDER BY LEAVEID DESC "));
    if(isset($query))
    {
  if($query->num_rows >= 1){
    ...
    while ($row = $query->fetch_object()){
        ...
        $student = <<<STAFF
                <tr>
                    ...
                    <button type="submit" id="cancel" name="cancel" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure,to cancel leave request')">Cancel</button>
                   </form>
               </td>
...
STAFF;
  ...
  {
    echo '<div class="mt-4">
        <h1 class="text-md text-center">No Requests are In-Process</h1></div>';
}
} 
