      ...
      <div class="topbar">
        ...
        <div class="sidebar">
            ...
            <div id="actions" class="list-group">
                  ...
                  <button type="button" class="list-group-item list-group-item-action" onclick="location.href='previous_approved_rejected.php'">Previous Actions</button>
        ...
        <div class="content">
             ...
             <nav aria-label="breadcrumb">
              <ol class="breadcrumb">
                <li class="breadcrumb-item active" aria-current="page">Approve/Reject Requests</li>
              </ol>
            </nav>
    ...
    
{
    ...
    if($_POST['LEAVESTATUS']== 'EDITED_REQUEST')
    {
            ...
            $query =  mysqli_query($link,("SELECT LEAVESTATUS FROM `tblleave` WHERE LEAVESTATUS='APPROVED' AND EMPID= '".$_POST['EMPID']."' ORDER BY LEAVEID DESC LIMIT 1"));
        ...
        mysqli_query($link,("UPDATE `tblleave` SET LEAVESTATUS = 'REJECTED' WHERE LEAVESTATUS='APPROVED' 
        AND EMPID= '".$_POST['EMPID']."'  ORDER BY LEAVEID DESC LIMIT 1 "));
     mysqli_query($link,("UPDATE `tblleave` SET LEAVESTATUS = 'INPROCESS' WHERE 
                                                                                        EMPID = '".$_POST['EMPID']."' 
    ...
    $query6 =  mysqli_query($link,("SELECT LEAVESTATUS,CL_USED,EL_USED,OD_USED,PERMISIONS__USED,LOP__USED,LATECOMING__USED,HALFCL__USED,HALFOD__USED,EXTRAPER__USED,EMPID FROM `tblleave` WHERE (LEAVESTATUS='INPROCESS' AND EMPID = '".$_POST['EMPID']."') 
                                  AND   LEAVEID= '".$_POST['LEAVEID']."' "));
    if(isset($query6))
    {
         ...
         $leave=mysqli_query($link,("SELECT CL,EL,OD,PERMISIONS,LOP,LATECOMING,EMPID from `tblemployee` WHERE 
                             EMPID = '".$_POST['EMPID']."' "));
 ...
 mysqli_query($link,(" UPDATE `tblleave` SET ADMINREMARKS='".mysqli_real_escape_string($link, $_POST['Reason'])."' WHERE LEAVEID = '".$_POST['LEAVEID']."' ") );
 ...
 else if($row2['EXTRAPER__USED']==0.0)
{
...
mysqli_query($link,("UPDATE `tblleave` SET LEAVESTATUS = 'APPROVED' WHERE 
                                                                                        EMPID = '".$_POST['EMPID']."' 
                                                                                  AND   LEAVEID= '".$_POST['LEAVEID']."' "));
    ...
    
{
    ...
    $query =  mysqli_query($link,("SELECT LEAVESTATUS FROM `tblleave` WHERE (LEAVESTATUS='INPROCESS' OR LEAVESTATUS='EDITED_REQUEST')"));
    
    if(isset($query))
    {
        
         $query = mysqli_query($link,("UPDATE `tblleave` SET LEAVESTATUS = 'REJECTED',ADMINREMARKS='".mysqli_real_escape_string($link, $_POST['Reason'])."' WHERE 
                                                                                        EMPID ='".$_POST['EMPID']."'
                                                                                   AND  LEAVEID= '".$_POST['LEAVEID']."' "));
...
mysqli_query($link,(" UPDATE `tblleave` SET ADMINREMARKS='".mysqli_real_escape_string($link, $_POST['Reason'])."' WHERE LEAVEID = '".$_POST['LEAVEID']."' ") );
    ...
    $query =  mysqli_query($link,("SELECT * FROM `tblleave` WHERE (LEAVESTATUS='INPROCESS' OR LEAVESTATUS='EDITED_REQUEST') ORDER BY LEAVEID DESC"));
    ...
    while ($row = $query->fetch_object()){
        ...
        $student = <<<STAFF
                <tr>
                    ...
                    <td><form  method="post" >
                         <textarea placeholder="REMARKS" cols="20" rows="2" class="reason" name="Reason" required></textarea><br>
                         <input type="hidden" name="EMPID" value="$row->EMPID" >
                         <input type="hidden" name="LEAVEID" value="$row->LEAVEID">
                         <input type="hidden" name="LEAVESTATUS" value="$row->LEAVESTATUS">
                         <button type="submit" id="accept " name="approve"  class="btn btn-success btn-sm" onclick="return confirm('Are you sure,to approve leave request')">Approve</button>
...
STAFF;
  ...
  {
    echo '<div class="mt-4">
        <h1 class="text-md text-center">No New Requests Available</h1></div>';
}
